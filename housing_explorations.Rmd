...
```{r cache = TRUE}
all_housing_data <-
  left_join(clean_zhvi, clean_sale_prices, by = c("region_id", "zip_code", "date")) %>% 
  full_join(clean_zri,
            by = c("region_id", "zip_code", "city", "state", "metro", "county_name", "date")) %>%
  full_join(clean_rental_prices,
            by = c("zip_code", "city", "state", "metro", "county_name", "date"))
...
mean_housing_data <- all_housing_data %>%
  ...
  full_join(clean_median_income, by = c("metro", "date")) %>% 
  pivot_longer(cols = c(mean_zhvi, mean_sale_price, mean_zri, mean_rental_price, median_income),
               names_to = "value_type",
               values_to = "mean_value")
```

```{r cache = TRUE}
income_zhvi <- clean_zhvi %>% 
  group_by(metro, date) %>% 
...
```{r}
mean_housing_data %>% write_csv(path = "housing-explorations-app/all_housing_data.csv")
median_income %>% write_csv(path = "housing-explorations-app/median_income.csv")

dir_create("graphics")
anim_save("graphics/graphic.gif", graphic)
file_copy("graphics/graphic.gif", "housing-explorations-app/graphic.gif")
```
